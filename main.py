...
PLAIN_TEXT: dict[str, dict[str, dict[str, str]]] = dict() 
...
def main() -> None:
    for i in range(1, 32):
        timetable = req.get(f'{URL}o{i}.html')
        timetable_html = bs(timetable.content, 'html.parser')
        grade = timetable_html.find('span', class_ = 'tytulnapis').text.split(' ')[0]
        print(grade)
        row: Tag
        for num_row, row in enumerate(timetable_html.find('table', class_='tabela').find_all('tr')[1:]):
            col: Tag
